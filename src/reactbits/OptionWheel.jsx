...
const OptionWheel = ({
  ...
  const runFrame = useCallback((now) => {
    const dt = Math.min((now - lastRef.current) / 1e3, 0.05);
    lastRef.current = now;
    ...
    const settled = Math.abs(target - next) < 1e-3;
  ...
  const playTick = useCallback(() => {
    const { soundUrl: soundUrl2, soundVolume: soundVolume2 } = cfgRef.current;
    if (!soundUrl2) return;
    const now = performance.now();
    if (now - lastTickRef.current < 70) return;
    lastTickRef.current = now;
    if (!audioRef.current || audioUrlRef.current !== soundUrl2) {
      audioRef.current = new Audio(soundUrl2);
      audioRef.current.preload = "auto";
      audioUrlRef.current = soundUrl2;
    }
    const audio = audioRef.current;
    audio.volume = Math.min(Math.max(soundVolume2, 0), 1);
    audio.currentTime = 0;
    audio.play()?.catch(() => {
    });
  }, []);
...
var OptionWheel_default = OptionWheel;
export {
  OptionWheel_default as default
};
