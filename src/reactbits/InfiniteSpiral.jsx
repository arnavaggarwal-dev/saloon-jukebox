...
const InfiniteSpiral = ({
  ...
  const normalizedItems = useMemo(
    () => items.map(
      (item, index) => typeof item === "string" ? { src: item, alt: `Spiral image ${index + 1}` } : { alt: `Spiral image ${index + 1}`, ...item }
    ),
    [items]
  );
  useEffect(() => {
    ...
    const render = (time) => {
      const delta = Math.min((time - previousTime) / 1e3, 0.05);
      previousTime = time;
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        card.style.zIndex = String(Math.round(depth * 1e5) + index);
...
var InfiniteSpiral_default = InfiniteSpiral;
export {
  InfiniteSpiral_default as default
};
