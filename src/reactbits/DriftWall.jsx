...
const DriftWall = ({
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = <span className={innerClass}>
        <img
      src={item.image}
      alt={item.title ?? ""}
      loading="lazy"
    ...
    if (item.href) {
      return <a key={id} href={item.href} target="_blank" rel="noreferrer noopener" {...commonProps}>
          {inner}
        </a>;
    }
    return <div key={id} tabIndex={0} role="button" aria-label={item.title ?? "tile"} {...commonProps}>
        {inner}
      </div>;
  };
  return <div
