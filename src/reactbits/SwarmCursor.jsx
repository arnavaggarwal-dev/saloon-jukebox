...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    const frame = (now) => {
      ...
      if (lastSample < 0 || nowSec - lastSample >= 8e-3) {
        lastSample = nowSec;
  ...
  return <div ref={containerRef} className={`relative w-full h-full ${className}`.trim()} style={style} {...rest}>
      {children ? <div className="absolute inset-0 flex items-center justify-center pointer-events-none">{children}</div> : null}
    </div>;
};
var SwarmCursor_default = SwarmCursor;
export {
  SwarmCursor_default as default
};
